...
void cmGlobalNinjaGenerator::WriteMainMakefile2()
{
  ...
  lg = static_cast<cmLocalNinjaGenerator *>(this->LocalGenerators[0]);
  lg->WriteSpecialTargetsBottom(makefileStream);
}


//----------------------------------------------------------------------------
void cmGlobalNinjaGenerator::WriteMainCMakefile()
{
  // Open the output file.  This should not be copy-if-different
  // because the check-build-system step compares the makefile time to
  // see if the build system must be regenerated.
                                  ...
                                  )
{
  ...
  for (unsigned int i = 0; i < lGenerators.size(); ++i)
    {
    lg = static_cast<cmLocalNinjaGenerator *>(lGenerators[i]);
                      ...
                      bool check_relink)
{
  ...
  for(std::vector<cmLocalGenerator*>::iterator sdi =
    ...
    {
    cmLocalNinjaGenerator* slg =
      static_cast<cmLocalNinjaGenerator*>(*sdi);
                       ...
                       const char* config, bool ignoreErrors, bool fast)
{
  ...
  if ( targetName && strlen(targetName))
    ...
    if (this->LocalGenerators.size())
      {
      lg = static_cast<cmLocalNinjaGenerator *>
...
size_t
cmGlobalNinjaGenerator
::CountProgressMarksInAll(cmLocalNinjaGenerator* lg)
{
  size_t count = 0;
  std::set<cmTarget*> emitted;
...
//----------------------------------------------------------------------------
void
cmGlobalNinjaGenerator::RecordTargetProgress(
...
//----------------------------------------------------------------------------
bool
cmGlobalNinjaGenerator::ProgressMapCompare
...
//----------------------------------------------------------------------------
void
cmGlobalNinjaGenerator::TargetProgress
...
//----------------------------------------------------------------------------
void cmGlobalNinjaGenerator::WriteHelpRule
(std::ostream& ruleFileStream, cmLocalNinjaGenerator *lg)
{
  // add the help target
  std::string path;
