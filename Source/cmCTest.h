...
public:
  ...
  /**
   * Should ctest configuration be updated. When using new style ctest
...
private:
  ...
  bool HandleTestActionArgument(
      const char* ctestExec, size_t &i, const std::vector<std::string> &args);

  /** Handle the --test-model command line argument */
  bool HandleTestModelArgument(
      const char* ctestExec, size_t &i, const std::vector<std::string> &args);
